...
func show_overlay(title: String, body: String) -> void:
	_e._dialog_title.text = title
	_e._dialog_body.text = body
	_e._dialog_body.visible = true
	if _e._dialog_input_row:
		_e._dialog_input_row.visible = false
	_e._dialog_panel.visible = true
	_e._overlay.visible = true
...
func show_input_dialog(title: String, body: String, placeholder: String, default_val: String, btn_label: String, callback: Callable, secret: bool = false) -> void:
	...
	_e._dialog_body.visible = true
	if _e._dialog_input_row:
		_e._dialog_input_row.visible = true
		_e._dialog_input.placeholder_text = placeholder
		_e._dialog_input.text = default_val
		_e._dialog_input.secret = secret
		_e._dialog_action_btn.text = btn_label
	_e._dialog_action_callback = callback
	_e._dialog_panel.visible = true
	_e._overlay.visible = true
	if _e._dialog_input:
		_e._dialog_input.grab_focus()
		_e._dialog_input.select_all()



func on_dialog_action_pressed() -> void:
	if _e._dialog_action_callback.is_valid():
		var val: String = _e._dialog_input.text.strip_edges() if _e._dialog_input else ""
		var cb: Callable = _e._dialog_action_callback
		_e._dialog_action_callback = Callable()
		hide_overlay()
		cb.call(val)
	else:
		hide_overlay()
...
func hide_overlay() -> void:
	_e._dialog_panel.visible = false
	_e._overlay.visible = false
	_e._dialog_action_callback = Callable()
	if _e._dialog_input:
		_e._dialog_input.secret = false
		_e._dialog_input.text = ""
...
func show_config() -> void:
	...
	var body = "[b]Settings[/b]\n\n• [b]Typography:[/b] system interface font · FiraCode in editor\n• [b]Workspace:[/b] %s\n• [b]Active Model:[/b] %s (NVIDIA NIM)\n• [b]API key:[/b] %s\n• [b]Status:[/b] %s\n\nUse Config → NVIDIA NIM API key… to change the stored key." % [
		_e._workspace_root,
		_e._ai_provider.replace("_", " ").to_upper(),
		key_state,
...
func send_os_notification(title: String, body: String, is_warning: bool = false) -> void:
	...
	_e._os_notify_generation += 1
	var gen = _e._os_notify_generation
	_e.get_tree().create_timer(expire_s).timeout.connect(func() -> void:
		if gen == _e._os_notify_generation:
...
func get_open_file_dlg() -> FileDialog:
	if _e._open_file_dlg == null:
		_e._open_file_dlg = FileDialog.new()
		_e._open_file_dlg.title = "Open a File"
		_e._open_file_dlg.size = Vector2i(800, 520)
		_e._open_file_dlg.file_mode = FileDialog.FILE_MODE_OPEN_FILE
		_e._open_file_dlg.access = FileDialog.ACCESS_FILESYSTEM
		_e._open_file_dlg.file_selected.connect(_e.files.open_path)
		_e.add_child(_e._open_file_dlg)
	return _e._open_file_dlg



func get_open_dir_dlg() -> FileDialog:
	if _e._open_dir_dlg == null:
		_e._open_dir_dlg = FileDialog.new()
		_e._open_dir_dlg.title = "Open a Directory"
		_e._open_dir_dlg.size = Vector2i(800, 520)
		_e._open_dir_dlg.file_mode = FileDialog.FILE_MODE_OPEN_DIR
		_e._open_dir_dlg.access = FileDialog.ACCESS_FILESYSTEM
		_e._open_dir_dlg.dir_selected.connect(_e.files.on_dir_selected)
		_e.add_child(_e._open_dir_dlg)
	return _e._open_dir_dlg



func get_save_as_dlg() -> FileDialog:
	if _e._save_as_dlg == null:
		_e._save_as_dlg = FileDialog.new()
		_e._save_as_dlg.title = "Save As"
		_e._save_as_dlg.size = Vector2i(800, 520)
		_e._save_as_dlg.access = FileDialog.ACCESS_FILESYSTEM
		_e._save_as_dlg.file_selected.connect(_e.files.save_as_path)
		_e.add_child(_e._save_as_dlg)
	return _e._save_as_dlg



func get_open_theme_xml_dlg() -> FileDialog:
	if _e._open_theme_xml_dlg == null:
		_e._open_theme_xml_dlg = FileDialog.new()
		_e._open_theme_xml_dlg.title = "Open Theme XML"
		_e._open_theme_xml_dlg.size = Vector2i(800, 520)
		_e._open_theme_xml_dlg.file_mode = FileDialog.FILE_MODE_OPEN_FILE
		_e._open_theme_xml_dlg.access = FileDialog.ACCESS_FILESYSTEM
		_e._open_theme_xml_dlg.filters = PackedStringArray(["*.xml ; Theme XML files"])
		_e._open_theme_xml_dlg.file_selected.connect(_e.themes.import_theme_from_xml)
		_e.add_child(_e._open_theme_xml_dlg)
	return _e._open_theme_xml_dlg
